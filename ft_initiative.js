...
async function handleInitCommand(msg, cmd, user) {
  ...
  // report
  msg.reply(await addMaintenanceStatusMessage(msg, output)).catch((e) => { logError(e); });
...
async function handleSetPlayersCommand(msg, args) {
  ...
  // remove reaction
  removeHourglass(msg);
  msg.reply(await addMaintenanceStatusMessage(msg, ` your group in channel <#${gmPlayChannelID}> is now ${args.length} players.`)).catch((e) => { logError(e); });
  // listAllFiles();
  logWrite(`🎲🎲 ${msg.channel.guild.id}/${msg.channel.id}(${gmPlayChannelID})/${msg.author.id}`);
}
async function handleAddPlayersCommand(msg, args) {
  ...
  try {
    if (gmPlayersFileID !== -1) {
      ...
      msg.reply(await addMaintenanceStatusMessage(msg, ` you added ${args.length} players `
...
async function handleSetNPCInitCommand(msg, args) {
  ...
  if (!await validateNPCInput(msg, args)) {
    return;
  }
  // and on to the show
